          ...
          {faqs.map((faq, index) => (
            <FadeIn key={index} delay={index * 0.1}>
              <div className="bg-white border border-gray-200">
                <button
                  className="w-full px-4 sm:px-6 py-4 sm:py-5 flex justify-between items-start sm:items-center text-left gap-4 focus:outline-none focus:ring-2 focus:ring-brand-purple focus:ring-inset"
                  onClick={() =>
                    setOpenIndex(openIndex === index ? null : index)
                  }
                >
                  <span className="font-heading text-lg sm:text-xl font-bold text-brand-asphalt uppercase pr-0 sm:pr-8">
                    {faq.question}
                  </span>
                  <ChevronDown
                ...
                >
                  <div className="px-4 sm:px-6 pb-6 text-sm sm:text-base text-gray-600 leading-relaxed">
                    {faq.answer}
                  </div>
                </div>
              </div>
